...
set_n <- 4 # simulation setting id number
...
saveRDS(x1, file = paste0(out_path, "/", "mar-pn-reg-", "mi-lm-sim_setting-", set_n, "-cca.rds"))

results <- x1 |> dplyr::bind_rows()

x2 <- x1 |> 
    dplyr::bind_rows()
f_out <- paste0(out_path, "/", "mar-pn-reg-", "mi-lm-sim_setting-", set_n, "-cca.csv")
